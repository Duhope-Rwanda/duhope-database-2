...
const TopSellingCategories: React.FC<Props> = ({ className, products, orders }) => {
      ...
      orders.forEach((order) => {
        if (order.products) {
          ...
          order.products.forEach((product) => {
            ...
            // Find the product in the products array to get the category name
            const productInfo = products.find((p) => p.id === productId);

            if (productInfo) {
              // Before setting the quantities, get the category name
              const categoryName = productInfo.category_id;
              ...
              if (existingProduct) {
                ...
                quantities.push({
                  ...
                  name: categoryName,  // Use category name from products array
                  image: productInfo.main_image_url || toAbsoluteUrl('/media/svg/avatars/001-boy.svg'),
                });
              }
            }
          });
    ...
    <div className={`card ${className}`}>
      <div className='card-header border-0 pt-5'>
        <h3 className='card-title align-items-start flex-column'>
          <span className='card-label fw-bold fs-3 mb-1'>Top selling Categories - Items sold</span>
        </h3>
      </div>
