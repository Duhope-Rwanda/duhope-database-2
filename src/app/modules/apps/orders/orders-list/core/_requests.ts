...
import { Order, UsersQueryResponse } from './_models'
...
const getUserById = (id: ID): Promise<Order | undefined> => {
  return axios
    .get(`${USER_URL}/${id}`)
...
const createUser = (user: Order): Promise<Order | undefined> => {
  return axios
    .put(USER_URL, user)
    .then((response: AxiosResponse<Response<Order>>) => response.data)
    .then((response: Response<Order>) => response.data)
...
const updateUser = (user: Order): Promise<Order | undefined> => {
  return axios
